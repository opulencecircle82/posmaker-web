// fix_weight_modal_styling.js
// The weight modal was built using dashboard-style CSS classes (.field,
// .row2, .btn, .btn-ghost, .btn-accent, <h3>) that don't exist in the
// cashier app's stylesheet — it uses its own classes (.mtitle, .inp,
// .q-cash, .btn-cancel, .btn-confirm). Also fixes closeModal() calls,
// which should be closeM() in this file. Rewrites with the correct classes.
// Run: node scripts/fix_weight_modal_styling.js
const fs = require('fs');
...
for (const file of files) {
  ...
  if (c.includes('class="mtitle" id="weightModalTitle"')) { console.log(`—  ${file}: already patched`); skipped++; continue; }
